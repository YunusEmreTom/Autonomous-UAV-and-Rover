...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # Kamera genişliğini ayarla
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # Kamera yüksekliğini ayarla

# Set FPS and resolution
input_size = (640, 480)  # Giriş boyutunu ayarla
fps = cap.get(cv2.CAP_PROP_FPS) if cap.get(cv2.CAP_PROP_FPS) > 0 else 30  # FPS'i ayarla
...
ffmpeg_cmd = [  # FFmpeg komut satırı argümanlarını ayarla
    'ffmpeg', '-f', 'rawvideo', '-pix_fmt', 'bgr24', '-s', '640x480', '-r', str(fps),
    '-i', '-', '-c:v', 'libx264', '-preset', 'ultrafast', '-tune', 'zerolatency',
    '-flags', '+cgop', '-vf', 'fps=fps=37', '-f', 'mpegts', 'udp://192.168.31.252:1553'
]
...
while cap.isOpened():  # Kamera açık olduğu sürece
    ...
    if key == ord('q'):  # 'q' tuşuna basıldıysa
        break  # Döngüden çık
    elif key == ord('r'):  # 'r' tuşuna basıldıysa
        is_uav_mode = not is_uav_mode  # UAV ve QR tespit modları arasında geçiş yap
